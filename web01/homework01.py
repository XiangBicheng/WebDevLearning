...
def host_of_url(url):
    ...
    返回代表主机的字符串, 比如 'g.cn'
    '''
    p = re.compile(r'(?P<protocol>https?://)?(?P<host>\w+(?P<hostext>[.]\w+)+)(?P<port>:\d+)?(?P<route>/.*)?')
    host = p.match(url).group('host')
    return host
...
def path_of_url(url):
    ...
    '''
    p = re.compile(r'(?P<protocol>https?://)?(?P<host>\w+(?P<hostext>[.]\w+)+)(?P<port>:\d+)?(?P<route>/.*)?')
    route = p.match(url).group('route') or '/'
    return route
...
def get(url):
    ...
    s.connect((host_of_url(url), port_of_url(url)))
    http_request = 'GET ' + path_of_url(url) + ' HTTP/1.1\r\nhost:' + host_of_url(url) + '\r\n\r\n'
